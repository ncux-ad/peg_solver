...
# Используйте наглядный скрипт:
./install_rust.sh
# или вручную:
# curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh

# Установите maturin (рекомендуется, с увеличенным таймаутом для медленного интернета)
...
**Примечание**: Если `maturin` не устанавливается из-за проблем с сетью, см. [rust_peg_solver/README_BUILD.md](rust_peg_solver/README_BUILD.md) для альтернативных методов сборки.
...
**Решение:**
1. **Убедитесь, что вы в venv**: `source venv/bin/activate`
2. Убедитесь, что Rust установлен: `rustc --version`
3. Убедитесь, что maturin установлен: `pip install --timeout=300 maturin`
4. Попробуйте пересобрать: `cd rust_peg_solver && maturin develop --release`
5. **Или используйте Cython/Python** (автоматический fallback) - вполне достаточно!

**Подробнее**: См. [rust_peg_solver/README_BUILD.md](rust_peg_solver/README_BUILD.md) для альтернативных методов сборки.

### Maturin не устанавливается (timeout/network)

**Проблема:** Ошибка при установке `maturin` через pip (timeout, connection error)

**Решение:**
1. Убедитесь, что вы в venv: `source venv/bin/activate`
2. Используйте увеличенный таймаут: `pip install --timeout=300 --retries=5 maturin`
3. Или соберите вручную через cargo (см. `rust_peg_solver/README_BUILD.md`)
4. **Или пропустите Rust** - используйте только Cython (26x ускорение - вполне достаточно!)
